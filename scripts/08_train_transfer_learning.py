#!/usr/bin/env python3
"""
08_train_transfer_learning.py - Transfer Learning with ResNet50

Uses pretrained ResNet50 (trained on ImageNet) for improved fish classification.
Two-stage training:
  Stage 1: Train only the classifier (backbone frozen) - 10 epochs
  Stage 2: Fine-tune entire model (all layers unfrozen) - 30 epochs

Why ResNet50?
- Proven performance on similar classification tasks
- Good balance between accuracy and speed
- 25M parameters (pretrained on 1.2M images)
- Excellent at learning fish features through transfer learning
"""
...
def get_data_transforms():
    """Create data transforms (ImageNet normalization)"""
...
def main():
    ...
    logger.info("\n" + "="*70)
    logger.info("STAGE 1: Training classifier (backbone frozen)")
    logger.info("="*70)
    ...
    for epoch in range(FREEZE_EPOCHS):
        ...
        if val_acc > best_val_acc:
            ...
            }, 'models/resnet50_stage1.pth')
            logger.info(f"  ✓ Saved (best val_acc: {val_acc:.2f}%)")

    # ============ STAGE 2: Fine-tune entire model ============
    logger.info("\n" + "="*70)
    logger.info("STAGE 2: Fine-tuning entire model (all layers unfrozen)")
    logger.info("="*70)
    ...
    for epoch in range(FINETUNE_EPOCHS):
        ...
        if val_acc > best_val_acc:
            ...
            torch.save({
                ...
                'val_acc': val_acc,
                'class_names': train_dataset.classes
            }, 'models/resnet50_transfer_best.pth')
    ...
    checkpoint = torch.load('models/resnet50_transfer_best.pth')
    ...
    logger.info(f"   Model saved: models/resnet50_transfer_best.pth")
